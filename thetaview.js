...
class ThetaView {
  ...
  _calcTexUv(i, j, lens) {
    ...
    const y =
      lens === 0
        ...
        : fishRad2 * (1.0 - ar.y) * Math.sin(-1.0 * ar.x + Math.PI) +
          fishCenter;
    ...
    for (let j = 0; j <= 180; j += 5) {
      for (let i = 0; i <= 360; i += 5) {
        geometry.vertices.push(
          new THREE.Vector3(
            Math.sin((Math.PI * j) / 180.0) *
              Math.sin((Math.PI * i) / 180.0) *
              500.0,
            Math.cos((Math.PI * j) / 180.0) * 500.0,
            Math.sin((Math.PI * j) / 180.0) *
              Math.cos((Math.PI * i) / 180.0) *
              500.0
          )
        );
      }
    ...
    for (let m = 0; m < 36; m++) {
      for (let n = 0; n < 72; n++) {
        ...
        geometry.faceVertexUvs[0].push(
          [uvs[t + 0], uvs[t + 1], uvs[t + 74]],
          [uvs[t + 1], uvs[t + 75], uvs[t + 74]]
        );
      }
    }
  ...
  _onMove(e, d) {
    ...
    this._lon =
      (this._onPointerDownPointerX - e.clientX) * d + this._onPointerDownLon;
    this._lat =
      (e.clientY - this._onPointerDownPointerY) * d + this._onPointerDownLat;
  }
  ...
  start(videoDOM) {
    ...
    this._scene.add(
      new THREE.Mesh(this._createGeometry(), this._createMaterial(videoDOM))
    );
